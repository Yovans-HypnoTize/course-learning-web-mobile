...
import { firstLetterToUppercase, toastAndroid } from '../../utils/Utils';
      ...
      <View style={styles.cardPlanDetailsContainer}>
        <View>
          <Text style={styles.fw700}>
            {currentData.plan.planSelection} Month
            {currentData.plan.planSelection > 1 && 's'} (
            {currentData.course.courseLevel})
          </Text>
          <Text style={[styles.mt5, styles.textGray]}>
            {firstLetterToUppercase(currentData.plan.planDescription)}
          </Text>
        </View>
        <View>
...
const PaymentScreen = ({ route }: any) => {
  ...
  return (
    <View style={styles.containerWrapper}>
      <GradientHeader title="Payment" returnPath="Subscription" />
      <ScrollView style={styles.container}>
        <View style={styles.ph20}>
          <Text style={[styles.mt10, styles.title]}>Selected Plans</Text>
              ...
              return (
                <View key={index}>
                  <View style={styles.rowHorizontallySpaceBetween}>
                    ...
                    <Text style={[styles.textBlue, styles.fw600]}>
                      {currentItem.plan.planSelection} Month
                      {currentItem.plan.planSelection > 1 && 's'} (
                      {currentItem.course.courseLevel})
                    </Text>
                  </View>
